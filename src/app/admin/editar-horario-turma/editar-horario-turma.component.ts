...
@Component({
  selector: 'app-editar-horario-turma',
  templateUrl: './editar-horario-turma.component.html',
  styleUrls: ['./editar-horario-turma.component.css'],
  providers: [TurmaBD, DisciplinaBD]
})
export class EditarHorarioTurmaComponent implements OnInit {
    ...
    this.route.params.subscribe((parametros: Params) => {
      this.turmaBD.getTurmaPorCodigo(parametros.id)
        .then((resp: any) => {
          ...
          this.turmaCodigo = parametros.id
          if (resp.turno === 'Manhã') {
  ...
  public salvarHorario(): void {
    ...
    this.horario = { segunda: [], terca: [], quarta: [], quinta: [], sexta: [] }
    ...
    console.log(this.horario.sexta)
    this.turmaBD.cadastroHorario(this.turmaCodigo, this.horario)
      .then(() => alert("Sucesso!"))

    let disciplinasHorario: any = { horario: [], diaDaSemana: '', disciplina: '' }
    this.limparHorarioSiciplinasTurma()
    for (let i = 0; i < 5; i++) {
      if (i === 0) {
  ...
  private limparHorarioSiciplinasTurma(): void {
    this.disciplinas.forEach(elemento => {
      this.disciplinaBD.limparHorario(elemento)
    })
  }
