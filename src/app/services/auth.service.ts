...
export class Autenticacao {
    ...
    public async autenticar(matricula: string, senha: string): Promise<any> {
        ...
        return firebase.auth().signInWithEmailAndPassword(matricula + "@dominioaluno.com", senha)
            ...
            .catch(() => {
                 return firebase.auth().signInWithEmailAndPassword(matricula + "@dominioprof.com", senha)
                    .then((resposta: any) => {
                        firebase.auth().currentUser.getIdToken()
                            .then((idToken: string) => {
                                this.token_id = idToken
                    ...
                    .catch(() => {
                        return firebase.auth().signInWithEmailAndPassword(matricula + "@dominioadmin.com", senha)
                            .then((resposta: any) => {
                                firebase.auth().currentUser.getIdToken()
                                    .then((idToken: string) => {
                                        this.token_id = idToken
